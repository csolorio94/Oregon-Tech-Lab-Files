...
namespace AirlineGUI
{
    public class Reserver : IReservable, IPersistable
    {
        ...
        public void Save(String file, bool[] seats)
        {
            ...
            while(ei < ECONOMY_CLASS_NUM_SEATS)
            {
                textOut.WriteLine(Convert.ToString(eseats[ei]));
                ei++;
            }
            while(fi < FIRST_CLASS_NUM_SEATS)
            {
                textOut.WriteLine(Convert.ToString(fseats[fi]));
                fi++;
            }
            textOut.Close();                   
